...
# -----------------------------------------------------------------------
# --- SVG Rendering and Output FUNCTIONS E6. snap_to_grid(geom, precision=2)  ---
# -----------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------------------
# --- Geometry Series: FUNCTIONS G9. refine_centerline_geometry(centerline_coords, width=2.1, resolution=0.5) ---
# ---------------------------------------------------------------------------------------------------------------
def refine_centerline_geometry(centerline_coords, width=2.1, resolution=0.5):
    """
    G9: Refines a cartpath centerline using a cubic spline fit.
    - centerline_coords: List of (x, y) tuples from OSM 'Way'
    - width: The final fixed width of the cartpath (meters)
    - resolution: Distance between vertices on the new smooth line (meters)
    """
    if len(centerline_coords) < 3:
        # Not enough points for a curve; return as a simple ribbon
        return LineString(centerline_coords).buffer(width/2, join_style=2, cap_style=2)

    # 1. Convert to numpy and remove duplicates
    pts = np.array(centerline_coords)
    # Remove consecutive duplicates that can crash the spline
    mask = np.ones(len(pts), dtype=bool)
    mask[1:] = np.any(diff := np.diff(pts, axis=0) != 0, axis=1)
    pts = pts[mask]

    # 2. Calculate distance along the path (the 't' parameter)
    dist = np.cumsum(np.sqrt(np.sum(np.diff(pts, axis=0)**2, axis=1)))
    dist = np.insert(dist, 0, 0)

    # 3. Fit the Spline (The Gaussian-style 4-vertex influence)
    # 'bc_type=natural' ensures the curve doesn't whip wildly at the ends
    cs_x = CubicSpline(dist, pts[:, 0], bc_type='natural')
    cs_y = CubicSpline(dist, pts[:, 1], bc_type='natural')

    # 4. Resample at high resolution
    num_samples = int(dist[-1] / resolution)
    t_new = np.linspace(0, dist[-1], max(num_samples, 10))
    smooth_pts = np.c_[cs_x(t_new), cs_y(t_new)]

    # 5. Create the ribbon with perfectly parallel edges (Mitre Join)
    smooth_line = LineString(smooth_pts)
    refined_ribbon = smooth_line.buffer(width/2, join_style=2, cap_style=2)
    
    return refined_ribbon
