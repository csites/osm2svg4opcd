...
* To use this program, here is the typical workflow. 1) Follow the OPCD workflow to build your Inner and Outer Lidar / DEM elevation maps in QGIS.  Google "OPCD None to Done" (V4 Toolset) and download any programs you need, QGIS, Inkscape, etc. Please validate your Inner and Outers QGIS Layers are setup correctly, as it is used to define the borders of the area we work with.  2) Use QGIS to export a Hillshade image of your course using the 'Calculate from: Layer' to define coordinates from your Inner or Outer layers.  This can be helpful because if you export your Hillshade as a "Geotiff" image (.tif), it includes all of the coordinates for the course you're trying to build.   With your Hillshade image typically matched to your 'Inner' layer, we can us then Utilities/Download_MapOSM.py to download the map.osm (an XML OpenStreetMap image of your course and the surrounding area).  You can also do the same as Hillshade with the QGIS XYZ Tile tool for Google, Bing, and even OpenStreetMap (although, sadly that OpenStreetMap overlay is in a completely wrong format, which is why this program exists)   If your Hillshade, Google, Bing, DEM, export was clipped to your inner/outer layer the run;  ``` python Utilities/Download_MapOSM --styles-file styles.json Hillshade.tif -D'''.   This will build the 'map.osm' needed for the area defined with the Inner/Outer area masks. If you like, you can also use the OpenStreetMap.org website to lookup your course and use the 'Export' button to create the map.osm.  Note: in the later you may need to adjust the window size manually using Longitude and Latitude adjustments in generic WGS84 coordinates.  The OpenStreetMap.org website has the advantage that the golf course is searchable, and if you feel up to it, you can even contribute a little time to updating or adding features not described.     
