...
def main():
    ...
    for way in document.findall('way'):
        ...
        if style_data['stroke_to_path']:
            stroke_width = float(style_data['attrs'].get('stroke-width', 1))
            radius = style_data['corner_radius']
            
            # NOTE: We pass style_data['attrs'] to convert_stroke_to_path for cap_style lookup
            path_d = convert_stroke_to_path(way_coords, stroke_width, style_data['attrs'], radius) 

            # Prepare new style string for the filled path (fill from old stroke)
            fill_color = style_data['attrs'].get('stroke', 'black')
            new_style_parts = []
            for k, v in style_data['attrs'].items():
                # Filter out stroke-related attributes that shouldn't be on the filled path
                if k not in ['stroke', 'stroke-width', 'stroke-linecap', 'stroke-linejoin', 'fill', 'corner_radius', 'z-order']:
                    new_style_parts.append(f'{k}="{v}"')
            new_style_parts.append(f'fill="{fill_color}"')
            new_style = ' '.join(new_style_parts)

            svg_element = f'<path d="{path_d}" {new_style} id="way_{way_id}_path"/>\n'

        else:
            # Standard polyline
            polyline_points = ' '.join([f"{x:.4f} {y:.4f}" for x, y in way_coords])
            svg_element = f'<polyline points="{polyline_points}" {style_data["svg_style"]} id="way_{way_id}"/>\n'
